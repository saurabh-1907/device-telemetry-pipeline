...
from telemetry_pipeline.messages import Authorization, ClearingRecord, auth_request, parse, AUTH_REQUEST_MTI
from telemetry_pipeline.reconcile import reconcile
...
def test_dual_message_and_reconciliation():
    auth = Authorization("tx-1", Decimal("5"), "USD", "device-1")
    mti, fields = parse(auth_request(auth))
    assert mti == AUTH_REQUEST_MTI and fields[2] == "tx-1"
    clear = ClearingRecord("tx-1", Decimal("5"), "USD", "device-1")
    assert reconcile([auth], [clear])[0].status == "matched"
    mismatch = ClearingRecord("tx-1", Decimal("6"), "USD", "device-1")
    assert reconcile([auth], [mismatch])[0].status == "amount_mismatch"
    assert reconcile([auth], [])[0].status == "missing_clearing"
    orphan = ClearingRecord("tx-2", Decimal("1"), "USD", "device-1")
    assert reconcile([], [orphan])[0].status == "orphan_clearing"
